<?php
// This is the images controller
session_start();

// Bring the outside resources
...
switch ($action) {
    case 'upload':
        ...
        if ($imageCheck) {
            $message = '<p id="rM">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p id="rM">You must select a vehicle and image file for the vehicle.</p>';
        } else {
            ...
            if ($result) {
                $message = '<p id="sM">The upload succeeded.</p>';
            } else {
                $message = '<p id="rM">Sorry, the upload failed.</p>';
            }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p id='sM'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p id='rM'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = '<p id="rM">Sorry, no images could be found.</p>';
        }
